...
const applyPopulate = (query, populateFields = []) => {
  return populateFields.reduce(
    (currentQuery, populateField) => currentQuery.populate(populateField),
    query
  );
};
...
const getTaskProjectContext = async (
  taskId,
  userId,
  { taskPopulate = [], projectPopulate = [] } = {}
) => {
  const task = await applyPopulate(Task.findById(taskId), taskPopulate);
  ...
  const project = await applyPopulate(
...
const getTaskById = async (req, res) => {
  try {
    ...
    const { task, project, error } = await getTaskProjectContext(
      taskId,
      req.user._id,
      {
        taskPopulate: [
        ...
        projectPopulate: [{ path: "members.user", select: "name profilePicture" }],
      }
    );
...
const updateSubTask = async (req, res) => {
  try {
    ...
    const { task, error } = await getTaskProjectContext(taskId, req.user._id);
...
const getActivityByResourceId = async (req, res) => {
  try {
    const { resourceId: taskId } = req.params;

    const { error } = await getTaskProjectContext(taskId, req.user._id);
